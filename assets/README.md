...
`assets/fonts/` ships **Barlow** (Regular + SemiBold), used for all on-screen
text (clock, ticker, quiz, spotlight card). It's licensed under the SIL Open
Font License 1.1 (`assets/fonts/OFL.txt`) — free for any use, including this
one, with no attribution required. It's bundled and registered directly with
`canvas`/the browser so text renders identically everywhere, including a bare
GitHub Actions runner that has no fonts installed system-wide.
